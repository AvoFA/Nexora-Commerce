...
import {
  Box, Typography, FormControl, InputLabel, Select, MenuItem, Modal, Fade, Backdrop, Button
} from '@mui/material';
  ...
  const handleInputChange = (field, value) => {
    setFormData(prev => {
      ...
      if (field === 'category' && (!prev.attributes || prev.attributes.length === 0)) {
        ...
        if (selectedCategory && selectedCategory.defaultAttributes && selectedCategory.defaultAttributes.length > 0) {
          newData.attributes = selectedCategory.defaultAttributes.map(attr => ({
            key: attr.key,
            value: attr.value
          }));
        }
        ...
        <Box className="admin-modal-wrapper">
          <div className="admin-modal-card admin-solid-card">
            ...
            <div className="admin-modal-content">
              <Box id="product-form" component="form" onSubmit={handleSave} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <FormControl className="mui-form-control" fullWidth>
                    <TextField
                      ...
                      onChange={(e) => {
                        ...
                        handleInputChange('price', val === '' ? '' : parseFloat(val) || '');
                      }}
                      InputProps={{
                        endAdornment: <Box sx={{ ml: 1 }}>грн</Box>,
                      }}
                      error={!!errors.price}
                  ...
                  <FormControl className="mui-form-control" fullWidth>
                    <TextField
                      ...
                      onChange={(e) => {
                        const val = e.target.value;
                        handleInputChange('stock', val === '' ? '' : parseInt(val) || '');
                      }}
                      error={!!errors.stock}
                ...
                <ImageUploadField
                  ...
                  onImageChange={(value) => handleInputChange('imageUrl', value)}
                />
            ...
            <div className="admin-modal-actions">
              <button type="button" onClick={handleClose} className="btn-secondary" disabled={isSaving}>
                Скасувати
              </button>
              <button type="submit" form="product-form" className="btn-primary" disabled={isSaving}>
